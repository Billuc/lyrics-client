...
from lyrics_client.classes.clients.azlyrics_lyrics_client import AZLyricsLyricsClient
from lyrics_client.classes.fetch_lyrics_command import FetchLyricsCommand


my_vcr = generate_vcr("test_azlyrics_client")

@my_vcr.use_cassette
def test_headers_session_and_xcode_initialized():
    client = AZLyricsLyricsClient()

    assert client.headers is not None
    assert client.session is not None
    assert client.x_code is not None
...
def test_client_name_is_azlyrics():
    ...
    assert client.client_name == "azlyrics"


@my_vcr.use_cassette
def test_search_songs():
    client = AZLyricsLyricsClient()
    command = FetchLyricsCommand("in the end", "linkin park", ["azlyrics"])
